...
$(document).ready(function () {
    ...
    $("#popupWindow").on('open', function () {
        $("#txtBlogCatName").jqxInput('selectAll');
    });
});
    ...
    $("#jqxgrid").jqxGrid(
        {
            ...
            columns: [
                ...
                { text: 'Name', datafield: 'CategoryName', width: "200px" },
                {
    ...
    $("#btnSave").click(function () {
        if (validateRequiredFields()) {
            if (editrow >= 0) {
                ...
                var dataSave = {
                    "Id": dataRecord.Id,
                    "CategoryName": $("#txtBlogCatName").val(),
...
function onhypEditClick(row) {
    ...
    $("#txtBlogCatName").val(dataRecord.CategoryName);
...
function onhypDeleteClick(row) {
    var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', row);
    if (confirm("Are you sure what to delete '" + dataRecord.CategoryName + "' Category?")) {
        var request = $.ajax({
...
function onhypDisplayOrderClick(id, upDown) {
    loadAll(RefreshData, "/CoreModule_General/ChangeBlogCategoriesDisplayOrder?id=" + id + "&upDown=" + upDown);
}
