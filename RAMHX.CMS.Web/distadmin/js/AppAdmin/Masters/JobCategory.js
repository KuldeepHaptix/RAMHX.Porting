...
$(document).ready(function () {
    ...
    $("#popupWindow").on('open', function () {
        $("#txtJobCatName").jqxInput('selectAll');
    });
});
    ...
    var source =
        {
            ...
            datafields: [
                { name: 'Id', type: 'int' },
                { name: 'JobCategoryName', type: 'string' },
    ...
    $("#jqxgrid").jqxGrid(
        {
            ...
            columns: [
                ...
                { text: 'Name', datafield: 'JobCategoryName', width: "200px" },
    ...
    $("#btnSave").click(function () {
        if (validateRequiredFields()) {
            if (editrow >= 0) {
                ...
                var dataSave = {
                    "Id": dataRecord.Id,
                    "JobCategoryName": $("#txtJobCatName").val(),
...
function onhypEditClick(row) {
    ...
    $("#txtJobCatName").val(dataRecord.JobCategoryName);
    $("#popupWindow").jqxWindow('open');
...
function onhypDeleteClick(row) {
    ...
    if (confirm("Are you sure what to delete '" + dataRecord.JobCategoryName + "' Category?")) {
        var request = $.ajax({
...
function onhypDisplayOrderClick(id, upDown) {
    loadAll(RefreshData, "/CoreModule_General/ChangeJobCategoryDisplayOrder?id=" + id + "&upDown=" + upDown);
}
