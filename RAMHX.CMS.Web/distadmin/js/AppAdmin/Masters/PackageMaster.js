...
var editor;
var packageAPIUrl = '/api/CoreModule_PackageMasters';
var categoryAPIUrl = '/api/CoreModule_PackageCategories';
...
function getCategory(CategoryId) {
    ...
    $.each(listCategory, function (index, item) {
        if (item.Id == CategoryId) {
            CategoryName = item.PackCategoryName
        }
    });
    return CategoryName;
}
...
function LoadCategory(response) {
    ...
    loadAll(BindData, packageAPIUrl);
}
...
function BindData(response) {
    ...
    $("#jqxgrid").jqxGrid(
        {
            ...
            columns: [
                ...
                { text: 'Name', datafield: 'PackName', width: "500px" },
    ...
    $("#popupWindow").on('open', function () {
        $("#txtPackName").jqxInput('selectAll');
    ...
    $("#btnSave").click(function () {
        ...
        if (validateRequiredFields()) {
            ...
            $.ajax({
                url: '/CoreModule_general/UploadFile?dir=' + '/files/package',
                data: formData,
                type: 'POST',
                async: false,
            ...
            if (editrow >= 0) {
                ...
                var request = $.ajax({
                    url: packageAPIUrl + "?id=" + dataRecord.Id,
                    cache: false,
...
function onhypEditClick(row) {
    ...
    $("#txtPackName").val(dataRecord.PackName);
...
function onhypDeleteClick(row) {
    ...
    if (confirm("Are you sure what to delete '" + dataRecord.PackName + "' Package?")) {
        var request = $.ajax({
            url: packageAPIUrl + "?id=" + dataRecord.uid,
            cache: false,
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            type: 'DELETE',
...
function onhypDisplayOrderClick(id, upDown) {
    loadAll(RefreshData, "/CoreModule_General/ChangePackageDisplayOrder?id=" + id + "&upDown=" + upDown);
}
