...
namespace RAMHX.CMS.Web.FileBrowser
{
    public partial class FileBrowser : System.Web.UI.Page, System.Web.UI.ICallbackEventHandler
    {
        ...
        public bool EditFile { get { return RAMHX.CMS.Infra.Common.QSBoolValue("edit"); } }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                HF_EditableFiles.Value = AppConfiguration.GetAppSettings("FileManager.Edit.SupportExtenstion");
                /**
                 * -------- Parameters --------------
                ...
                if (!string.IsNullOrEmpty(allowedFolder) && !SiteContext.CurrentUser_IsAdmin)
                {
                    ...
                    foreach (var fldr in folders)
                    {
                        ...
                        rp.LargeImageUrl = "~/FileBrowser/img/32/folder-document-alt.png";
                        rp.SmallImageUrl = "~/FileBrowser/img/16/folder-document-alt.png";
        ...
        public void RaiseCallbackEvent(String eventArgument)
        {
            ...
            switch (cmds[0].ToLower())
            {
                ...
                case "upload":
                    ...
                    break;
                case "extracthere":
                    string folderPath = Server.MapPath(VirtualPathUtility.AppendTrailingSlash(FileManager1.CurrentDirectory.VirtualPath));

                    if (!string.IsNullOrEmpty(cmds[1]) && cmds[1].ToLower().Contains(".zip"))
                    {

                        ZipFile.ExtractToDirectory(folderPath + cmds[1], folderPath.Trim('/'));
                        ajaxResponse.data = "Files are succesfully Extracted at " + folderPath;
                    }
                    else
                        ajaxResponse.data = "Selected File is Invalid, Please select Zip File";

                    string eventArgument1 = eventArgument;
                    break;
                default:
                    break;
            }
