...
$(document).ready(function () {
    ...
    $("#popupWindow").on('open', function () {
        $("#txtEventCatName").jqxInput('selectAll');
    ...
    $("#jqxgrid").jqxGrid(
        {
            ...
            columns: [
                ...
                { text: 'Name', datafield: 'EventName', width: "200px" },
                {
...
function onhypEditClick(row) {
    ...
    $("#txtEventCatName").val(dataRecord.EventName);
...
function onhypDeleteClick(row) {
    var dataRecord = $("#jqxgrid").jqxGrid('getrowdata', row);
    if (confirm("Are you sure what to delete '" + dataRecord.EventName + "' Category?")) {
        var request = $.ajax({
...
function onhypDisplayOrderClick(id, upDown) {
    loadAll(RefreshData, "/CoreModule_General/ChangeEventCategoriesDisplayOrder?id=" + id + "&upDown=" + upDown);
}
