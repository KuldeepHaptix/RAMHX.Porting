...
$(document).ready(function () {
    ...
    $("#popupWindow").on('open', function () {
        $("#txtProdCatName").jqxInput('selectAll');
    });
});
    ...
    var source =
        {
            ...
            datafields: [
                { name: 'Id', type: 'int' },
                { name: 'ProdCategoryName', type: 'string' },
    ...
    $("#jqxgrid").jqxGrid(
        {
            ...
            columns: [
                ...
                { text: 'Name', datafield: 'ProdCategoryName', width: "200px" },
    ...
    $("#btnSave").click(function () {
        if (validateRequiredFields()) {
            if (editrow >= 0) {
                ...
                var dataSave = {
                    "Id": dataRecord.Id,
                    "ProdCategoryName": $("#txtProdCatName").val(),
...
function onhypEditClick(row) {
    ...
    $("#txtProdCatName").val(dataRecord.ProdCategoryName);
    $("#popupWindow").jqxWindow('open');
...
function onhypDeleteClick(row) {
    ...
    if (confirm("Are you sure what to delete '" + dataRecord.ProdCategoryName + "' Category?")) {
        var request = $.ajax({
...
function onhypDisplayOrderClick(id, upDown) {
    loadAll(RefreshData, "/CoreModule_General/ChangeProdCategoryDisplayOrder?id=" + id + "&upDown=" + upDown);
}
