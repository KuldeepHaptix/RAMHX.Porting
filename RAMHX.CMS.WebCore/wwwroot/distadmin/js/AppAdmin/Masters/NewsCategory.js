...
$(document).ready(function () {
    ...
    $("#popupWindow").on('open', function () {
        $("#txtNewsCatName").jqxInput('selectAll');
    });
});
    ...
    $("#btnSave").click(function () {
        if (validateRequiredFields()) {
            if (editrow >= 0) {
                ...
                var dataSave = {
                    "Id": dataRecord.Id,
                    "CategoryName": $("#txtNewsCatName").val(),
...
function onhypEditClick(row) {
    ...
    $("#txtNewsCatName").val(dataRecord.CategoryName);
...
function onhypDisplayOrderClick(id, upDown) {
    loadAll(RefreshData, "/CoreModule_General/ChangeNewsCategoryDisplayOrder?id=" + id + "&upDown=" + upDown);
}
