...
$(document).ready(function () {
    ...
    $("#popupWindow").on('open', function () {
        $("#txtPackCatName").jqxInput('selectAll');
    });
});
    ...
    var source =
        {
            ...
            datafields: [
                { name: 'Id', type: 'int' },
                { name: 'PackCategoryName', type: 'string' },
    ...
    $("#jqxgrid").jqxGrid(
        {
            ...
            columns: [
                ...
                { text: 'Name', datafield: 'PackCategoryName', width: "200px" },
    ...
    $("#btnSave").click(function () {
        if (validateRequiredFields()) {
            if (editrow >= 0) {
                ...
                var dataSave = {
                    "Id": dataRecord.Id,
                    "PackCategoryName": $("#txtPackCatName").val(),
...
function onhypEditClick(row) {
    ...
    $("#txtPackCatName").val(dataRecord.PackCategoryName);
    $("#popupWindow").jqxWindow('open');
...
function onhypDeleteClick(row) {
    ...
    if (confirm("Are you sure what to delete '" + dataRecord.PackCategoryName + "' Category?")) {
        var request = $.ajax({
...
function onhypDisplayOrderClick(id, upDown) {
    loadAll(RefreshData, "/CoreModule_General/ChangePackCategoryDisplayOrder?id=" + id + "&upDown=" + upDown);
}
