...
$(document).ready(function () {
    ...
    $("#popupWindow").jqxWindow({
        width: 350,height: 250, resizable: false, isModal: true, autoOpen: false, cancelButton: $("#btnCancel"), modalOpacity: 0.01, theme: jqxTheme
    });
    $("#popupWindow").on('open', function () {
        $("#txtProjCatName").jqxInput('selectAll');
    });
});
    ...
    var source =
        {
            ...
            datafields: [
                { name: 'Id', type: 'int' },
                { name: 'ProjCategoryName', type: 'string' },
    ...
    $("#jqxgrid").jqxGrid(
        {
            ...
            columns: [
                ...
                { text: 'Name', datafield: 'ProjCategoryName', width: "200px" },
    ...
    $("#btnSave").click(function () {
        if (validateRequiredFields()) {
            if (editrow >= 0) {
                ...
                var dataSave = {
                    "Id": dataRecord.Id,
                    "ProjCategoryName": $("#txtProjCatName").val(),
...
function onhypEditClick(row) {
    ...
    $("#txtProjCatName").val(dataRecord.ProjCategoryName);
    $("#popupWindow").jqxWindow('open');
...
function onhypDeleteClick(row) {
    ...
    if (confirm("Are you sure what to delete '" + dataRecord.ProjCategoryName + "' Category?")) {
        var request = $.ajax({
...
function onhypDisplayOrderClick(id, upDown) {
    loadAll(RefreshData, "/CoreModule_General/ChangeProjCategoryDisplayOrder?id=" + id + "&upDown=" + upDown);
}
