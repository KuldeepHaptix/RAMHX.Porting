    ...
    public class SQLDataAccess
    {
        public static string GetAppSettings(string itemcode)
        {
            try
            {
                ...
                var app = dataContext.app_Configs.Where(itm => itm.ItemCode == itemcode && itm.GroupId == 1).FirstOrDefault();
                if (app != null)
                {
                    return app.ItemDesc;
                }
        ...
        public static void TakeAutoDailyBackup(bool takeNow)
        {
            ...
            try
            {
                var path = Path.GetDirectoryName(HttpContext.Current.Server.MapPath("/")) + "\\dbbackups\\" + DateTime.Now.ToString("DB_yyyyMMdd.bak");
                if (takeNow)
                {
                    path = Path.GetDirectoryName(HttpContext.Current.Server.MapPath("/")) + "\\dbbackups\\" + DateTime.Now.ToString("DB_yyyyMMdd_HHmmss.bak");
                }
                ...
                {
                    DatabaseEntities dataContext = new DatabaseEntities();
                    using (SqlCommand cmd = new SqlCommand("Backup database " + dataContext.Database.Connection.Database + "  to disk='" + path + "'"))
                    {
        ...
        public void ExecuteScript(cms_UpgradeHistory release)
        {
            DatabaseEntities dataContext = new DatabaseEntities();
            if (release.Script.ToLower() == "version.1.0.0.0.sql" && !HasInitialDB())
            {
                ...
                if (errors.Count == 0)
                {
                    ...
                    dataContext.cms_UpgradeHistory.Add(release);
            ...
            {
                var existUpgrade = dataContext.cms_UpgradeHistory.FirstOrDefault(rel => rel.Script == release.Script);
        ...
        private bool HasInitialDB()
        {
            ...
            {
                DatabaseEntities dataContext = new DatabaseEntities();
                var existUpgrade = dataContext.cms_UpgradeHistory.First();
                return true;
            }
            catch { }
            return false;
